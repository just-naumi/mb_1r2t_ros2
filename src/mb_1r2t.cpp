...
{
    if (m_scan_results.empty()) {
        return;
    }

    // Fixed number of beams (720 beams = 0.5 deg resolution)
    const int num_beams = 720;
    std::vector<float> ranges(num_beams, std::numeric_limits<float>::infinity());
    std::vector<float> intensities(num_beams, 0.0);

    for (const ScanResult& s : m_scan_results) {
        // Normalize angle to 0 .. 2*PI
        float angle = s.angle;
        while (angle < 0.0f) angle += 2.0f * M_PI;
        while (angle >= 2.0f * M_PI) angle -= 2.0f * M_PI;

        int idx = (int)(angle / (2.0f * M_PI / num_beams));
        if (idx >= 0 && idx < num_beams) {
            // Keep closest obstacle distance
            if (s.distance < ranges[idx] || std::isinf(ranges[idx])) {
                ranges[idx] = s.distance;
                intensities[idx] = (float)s.intensity;
            }
        }
    }

    m_laser_scan_msg.ranges = std::move(ranges);
    m_laser_scan_msg.intensities = std::move(intensities);

    rclcpp::Duration scan_time = now() - m_last_scan_time;

    m_laser_scan_msg.scan_time = scan_time.seconds();
    m_laser_scan_msg.time_increment = scan_time.seconds() / (float)num_beams;
    m_laser_scan_msg.angle_min = 0.0f;
    m_laser_scan_msg.angle_max = 2.0f * M_PI - (2.0f * M_PI / num_beams);
    m_laser_scan_msg.angle_increment = 2.0f * M_PI / num_beams;
